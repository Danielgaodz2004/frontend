...
import {T_Code, T_CodesListResponse, T_CreateCode} from "modules/types.ts";
import {AsyncThunkConfig} from "@reduxjs/toolkit/dist/createAsyncThunk";
import {api} from "modules/api.ts";
import {AxiosResponse} from "axios";
import {saveCalculation} from "store/slices/calculationsSlice.ts";
import { Code } from "src/api/Api";
...
export const createCode = createAsyncThunk<void, T_CreateCode, AsyncThunkConfig>(
    "codes/create_code",
    async function(data, {getState}) {
        const state = getState()
        await api.codes.codesCreateCreate(data=data)
    }
)
...
export const deleteCode = createAsyncThunk<T_Code[], string, AsyncThunkConfig>(
    "delete_code",
    async function(id) {
        const response = await api.codes.codesDeleteDelete(id) as AxiosResponse<T_Code[]>
        return response.data
    }
)
